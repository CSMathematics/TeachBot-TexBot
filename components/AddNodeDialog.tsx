...
import { createNode } from '../services/syllabusService';
...
const AddNodeDialog: React.FC<AddNodeDialogProps> = ({ isOpen, onClose, type, parentId, syllabusId, onSuccess }) => {
    const [title, setTitle] = useState('');
    const [contentType, setContentType] = useState<'THEORY' | 'METHODOLOGY'>('THEORY');
    ...
    const handleAiSuggest = async () => {
        ...
        try {
            ...
            console.error('AI suggest for prerequisites failed', err);
    ...
    const handleSave = async () => {
        ...
        try {
            await createNode(
                ...
                title,
                type,
                type === 'PARAGRAPH' ? contentType : undefined,
                (type === 'CHAPTER' || type === 'SECTION') && prerequisites.trim() ? prerequisites.trim() : undefined
            );
            onSuccess();
            onClose();
            setTitle(''); // Reset form
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    <DialogTitle>Προσθήκη {typeLabels[type]}</DialogTitle>
                </DialogHeader>
                <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <Input
                            ...
                            placeholder={`Όνομα ${typeLabels[type]}...`}
                            autoFocus
                        />
                    </div>
                        ...
                        <div className="grid grid-cols-4 items-center gap-4">
                            ...
                            <div className="col-span-3 flex gap-4">
                                <div className="flex items-center space-x-2">
                                    ...
                                    />
                                    <Label htmlFor="theory">Θεωρία</Label>
                                </div>
                                <div className="flex items-center space-x-2">
                                    <input
                                        type="radio"
                                        ...
                                        className="accent-primary"
                                    />
                                    <Label htmlFor="methodology">Μεθοδολογία</Label>
                                </div>
                            </div>
                        </div>
                    )}

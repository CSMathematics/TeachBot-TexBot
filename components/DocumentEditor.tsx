...
export const DocumentEditor: React.FC<DocumentEditorProps> = ({
    ...
    return (
        <div className={`flex flex-col gap-4 h-full ${className}`}>
            ...
            {!hideToolbar && (
                <div className="flex items-center justify-between bg-white dark:bg-slate-800 p-2 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                        {/* LatexFixer removed for compactness - can be re-added as a modal later */}
            ...
            <div className="flex-1 flex gap-4 min-h-0">
                ...
                <div className={`flex-1 flex flex-col min-h-0 transition-all duration-300 ${showPreview ? 'hidden md:flex' : 'flex'}`}>
                    <div className="bg-slate-900 text-slate-200 rounded-t-md p-2 text-xs flex justify-between items-center shrink-0">
                ...
                <div className={`flex-1 flex flex-col min-h-0 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden transition-all duration-300 ${!showPreview ? 'hidden' : 'flex'}`}>
                    <div className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 p-2 flex justify-between items-center shrink-0">
